...
func (c *Compiler) Compile(node *parser.Node) (code string) {
	...
	for i := 0; i < len(node.Children); i++ {
		...
		case *parser.IfBlock:
			...
			// 生成if条件判断的代码
			code += Format("\033[35mcmp\033[0m \033[34meax\033[0m, 0; 比较条件\n")
			if ifBlock.Else {
				code += Format("\033[35mje\033[0m else_" + label + "\033[32m; 如果条件为假，跳转到else部分\n")
			} else {
				code += Format("\033[35mje\033[0m end_" + label + "\033[32m; 如果条件为假，跳转到endif部分\n")
			}
			// 生成if块的代码
			for _, child := range node.Children {
				code += c.Compile(child)
			}
			if ifBlock.Else {
				// 生成else部分的标签
				code += Format("\033[35mjmp\033[0m end_" + label + "\033[32m; 跳过else部分\n")
				code += Format("else_" + label + ":\n")
				// 生成else块的代码
				if ifBlock.ElseBlock != nil {
					for _, child := range ifBlock.ElseBlock.Children {
						code += c.Compile(child)
					}
				}
			}
		...
		case *parser.CallBlock:
			...
			/*for _, arg := range n.Value.(*parser.CallBlock).Args {
				code += Format("\033[35mmov\033[0m " + arg.Name + ", \033[32m; 设置参数\n")
			}*/
			code += Format("\033[35mcall\033[0m " + n.Value.(*parser.CallBlock).Name + "\033[32m; 调用函数\n")
		}
	}
